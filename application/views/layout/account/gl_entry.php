                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                    <h4 class="mb-sm-0 font-size-18"><?=isset($moduleTitle)?$moduleTitle:''?> GL Entry</h4>
                        ...
                        <div class="row">
                            <div class="col-xl-12">
                                <div class="card" style="-webkit-box-shadow: unset;">
                                    <div class="card-body">
                                        <!-- <div class="py-2 mt-3">
                                            <h3 class="font-size-15 fw-bold"><?=isset($moduleTitle)?$moduleTitle:''?> GL Entry</h3>
                                        </div> -->
                                        <div class="row d-print-none">
                                                    ...
                                                    <label for="validationCustom05" class="form-label">Choose Business Unit</label>
                                                        <select class="form-control select2" name="GLJH_BUS_UNIT">
                                                            ...
                                                            <?php foreach ($busUnits as $busUnit):?>
                                                                    <option value="<?=$busUnit->BU_CODE?>" <?php if($glEntryId){ echo $glDet[0]->GLJH_BUS_UNIT == $busUnit->BU_CODE?'selected':null; }else{ echo defaultBusUnit() == $busUnit->BU_CODE ? 'Selected':null;}?>><?=$busUnit->BU_NAME1?></option>
                                                            <?php endforeach; ?>
                                                        </select>
                                            ...
                                            <div class="mb-3 col-lg-2">
                                                ...
                                                <select name="GLJH_JOURNAL_PFX" class="form-control select2" onchange="glPrefixCh(this)" <?=$glEntryId?'disabled':null?>>
                                                    <option value='' Selected disabled>Select</option>
                                                    <?php foreach ($prefixs as $prefixsGet): ?>
                                        
                                                        <option value="<?=$prefixsGet->GLP_JOURNAL_PFX?>" orderCount="<?=$prefixsGet->GLP_NEXT_NUMBER?>" desc="<?=$prefixsGet->GLP_DESC?>" <?php if($glEntryId){ echo $glDet[0]->GLJH_JOURNAL_PFX == $prefixsGet->GLP_JOURNAL_PFX?'selected':null; }?>><?=$prefixsGet->GLP_JOURNAL_PFX?></option>
                                                        
                                                    <?php endforeach;?>
                                                </select>
                                                <label id="GLJH_JOURNAL_PFX-error" class="error"></label>
                                                <label class='gl-prefix-desc'></label>
                                            </div>
                                            <div class="mb-3 col-lg-2">
                                                <label for="name">Journal No</label>
                                                <input type="text" class="form-control inc-count" name="GLJH_JOURNAL_NO" value="<?=$glEntryId?$glDet[0]->GLJH_JOURNAL_NO:null?>" placeholder="<?php if(isset($moduleType)){ if($moduleType == 'INV'){ echo 'MT';}else{ echo 'INVOICE';}}?> NO" aria-describedby="option-date" readonly>
                                                <label id="GLJH_JOURNAL_NO-error" class="error"></label>
                                            </div>
                                            <div class="mb-3 col-lg-2">
                                                <label for="name">GL Period</label>
                                                <div class="input-group mb-3">
                                                    <span class="input-group-text gl-period-mon-dsp" id="option-date"><?=$glEntryId?$glDet[0]->GLJH_PERIOD:date('Y')?></span>
                                                    <input type="text" class="form-control"
                                                        aria-describedby="option-date" disabled="true" value="<?=$glEntryId?$glDet[0]->GLJH_YEAR:date('Y')?>">
                                                </div>
                                            </div>
                                            <div class="mb-3 col-lg-2">
                                                <label for="name">Journal Date</label>
                                                <input type="date" class="form-control" name="gl_date_db" value="<?=$glEntryId?$glDet[0]->GLJH_JOURNAL_DATE:date('Y-m-d')?>">
                                            </div>
                                            <div class="mb-3 col-lg-2">
                                                <label for="name">Journal Reference</label>
                                                <input type="text" class="form-control" name="GLJH_JOURNAL_REF" value="<?=$glEntryId?$glDet[0]->GLJH_JOURNAL_REF:null?>">
                                            </div>
                                            <!-- <div class="mb-3 col-lg-2">
                                        ...
                                        <div class="row d-print-none">
                                            <div class="mb-3 col-lg-6">
                                                ...
                                                <textarea class="form-control" name="GLJH_DESC" rows="4" cols="50"><?=$glEntryId?$glDet[0]->GLJH_DESC:null?></textarea>
                                        ...
                                        <div class="d-print-none">
                                            <div class="float-end">
                                                ...
                                                <?php
                                                    if($glEntryId){
                                                        if($glDet[0]->GLJH_JOURNAL_ACTION == 'N'){ ?>
                                                            <button data-aftreload="<?=$glEntryId?"false":"true"?>" data-control="account/gl-entry-db" data-form="form-data" data-sweetalert="<?=$glEntryId?$sweetAlertMsg->glEntryPost->msg:$sweetAlertMsg->glEntryAdd->msg?>" data-sweetalertcontrol="<?=$glEntryId?$sweetAlertMsg->glEntryPost->cont:$sweetAlertMsg->glEntryAdd->cont?>" class="ajaxform btn btn-primary w-md waves-effect waves-light" disabled><?=$glEntryId?'UPDATE':'SAVE'?></button>
                                                     <?php }}else{ ?>
                                                        <button data-aftreload="<?=$glEntryId?"false":"true"?>" data-control="account/gl-entry-db" data-form="form-data" data-sweetalert="<?=$glEntryId?$sweetAlertMsg->glEntryPost->msg:$sweetAlertMsg->glEntryAdd->msg?>" data-sweetalertcontrol="<?=$glEntryId?$sweetAlertMsg->glEntryPost->cont:$sweetAlertMsg->glEntryAdd->cont?>" class="ajaxform btn btn-primary w-md waves-effect waves-light" disabled><?=$glEntryId?'UPDATE':'SAVE'?></button>
                                                    <?php }?>
                                                
                                                <span id="outmsg"></span></br>
                                                <input type="hidden" name="gl_pre_db" value="<?=$glEntryId?$glDet[0]->GLJL_JOURNAL_PFX:null?>">
                                                <input type="hidden" name="gl_order_db" value="<?=$glEntryId?$glDet[0]->GLJL_JOURNAL_NO:null?>">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    <?php
        ...
        <?php  foreach ($glDet as $glDetGet) { ?>

                tableLength = $('#tr-append tr').length+1;
                    ...
                    $('#tr-append').append(`
                        <tr 
                            ...
                            <td>${tableLength}</td>
                            <td>
                                <input type="text" class="form-control" onInput="accDet(this)" onPaste="accDet(this)" placeholder="Search Account" value="<?=$glDetGet->AH_SUBSIDERY?>">
                                <label class='acc-desc'><span class='text-success'><?=$glDetGet->AR_Title.'-'.$glDetGet->EN_Title.'-'.$glDetGet->AH_SUBSIDERY?></span></label>
                                <input type="hidden" name="acc_no_db[]" id="acc-det-db" value="<?=$glDetGet->AH_SUBSIDERY?>">
                                <label id="acc-no-error" class="error"></label>
                            </td>
                            <td>
                                <textarea class="form-control acc-desc-in" name="acc_desc_db[]" rows="2" cols="50"><?=$glDetGet->GLJL_DESC?></textarea>
                                <label id="acc-desc-error" class="error"></label>
                            </td>
                            <td><input type="text" name="debit_val_db[]" class="form-control tr-debit-in" onBlur="tbCal()" value="<?=$glDetGet->GLJL_DEBIT_AMT?>"></td>
                            <td><input type="text" name="credit_val_db[]" class="form-control tr-credit-in" onBlur="tbCal()" value="<?=$glDetGet->GLJL_CREDIT_AMT?>"></td>
                            <td>
                                <select class="form-control cost-center-in" onChange="costCent(this)" <?=$glDetGet->GLJL_COST_CENTER == 'N'?'disabled':null?>>
                                    <option value='' Selected disabled>Select</option>
                                    <?php foreach ($costCentDet as $costCentDetGet):?>
                                        <option value="<?=$costCentDetGet->CC_CODE?>" <?=$glDetGet->GLJL_COST_CENTER == $costCentDetGet->CC_CODE?'selected':null?>><?=$costCentDetGet->CC_CODE?></option>
                                    <?php endforeach; 
                                    if($glDetGet->GLJL_COST_CENTER == 'N'){ ?>
                                        <option value="N" Selected></option>
                                    <?php } ?>
                                    
                                </select></br>
                                <label id="cost-cent-error" class="error"></label>
                                <input type="hidden" name="cost_center_db[]" class="form-control cost-center-in-db" value="<?=$glDetGet->GLJL_COST_CENTER?>">
                            </td>
                            <td>
                                <input type="text" name="doc_date_db[]" class="form-control" value="<?=$glDetGet->GLJL_DOC_DATE?>">
                            </td>
            ...
            $('#tr-append').append(`
                                    <tr 
                                        ...
                                        <td>
                                            ...
                                            <input type="hidden" name="acc_no_db[]" id="acc-det-db">
                                            <label id="acc-no-error" class="error"></label>
                                        </td>
                                        <td>
                                            <textarea class="form-control acc-desc-in" name="acc_desc_db[]" rows="2" cols="50"></textarea>
                                            </br>
                                            <label id="acc-desc-error" class="error"></label>
                                        </td>
    ...
    function glPrefixCh(ele){
        ...
        $('.inc-count').val(myTag);
        $('.gl-prefix-desc').html(`<span class='text-primary'>${desc}</span>`);
    }
